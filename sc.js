const API_BASE='https://api.nasa.gov/planetary/apod';
  ...
  mDownload.onclick=()=>downloadFile(item.hdurl||item.url,item.title+'.jpg');
  ...
  document.body.style.overflow='hidden';
    ...
    async function downloadFile(url,name){try{const res=await fetch(url);const blob=await res.blob();const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=name;a.click();}catch(e){alert('Download failed');}}
